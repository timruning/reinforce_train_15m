...
def Position_Embedding(inputs, position_size):
    ...
    tmp = tf.expand_dims(position_ij, 0)
    position_embedding = tf.expand_dims(position_ij, 0) \
                         + tf.zeros((batch_size, seq_len, position_size))
    return position_embedding, position_j, position_i, position_ij
...
'''
普通的全连接
inputs是一个二阶或二阶以上的张量，即形如(batchz_size,...,input_size)。
...
def Attention2(Q, K, V):
    with tf.name_scope(name="attention"):
        shape_q = tf.shape(Q)
        Q = tf.reshape(Q, shape=(shape_q[0], shape_q[1], shape_q[2] * 64))
        shape_k = tf.shape(K)
        K = tf.reshape(K, shape=(shape_k[0], shape_k[1], shape_k[2] * 64))
        shape_v = tf.shape(V)
        V = tf.reshape(V, shape=(shape_v[0], shape_v[1], shape_v[2] * 64))
        A = tf.matmul(Q, K, transpose_b=True) / tf.sqrt(float(16 * 64))
        A = tf.check_numerics(A, message="Attention2_A")
        A = tf.nn.softmax(A)
        A = tf.check_numerics(A, message="Attention2_A2")
        O = tf.matmul(A, V)
        O = tf.check_numerics(O, message="Attention2_O")
        O = tf.reshape(O, shape=(shape_q[0], shape_q[1], 16 * 64))
    return O
